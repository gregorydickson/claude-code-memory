...
|---|---|---|---|---|
| 1 | Node-portable or clean static binary | ⛔ | Abstract Bun APIs: `bun:sqlite`→`node:sqlite`/`better-sqlite3`; `Bun.file`→`fs`; `Bun.sleep`→timers. Add `node` to `engines`, replace `#!/usr/bin/env bun`. Decide: in-process Node module **or** committed self-contained binary as the pinned surface. **Gated by D1.** | `sqlite.ts:9`, `cloud.ts:280`, `export-import.ts:78`, `package.json:26-28`, `cli.ts:1` |
| 2 | Critical security findings closed | 🟡 | Delete dead `safeExecSync`; add regression test proving no shell-string git path. Optional: containment check on `--output`. (Live RCE does not exist — see §0.) | `project-analysis.ts:156` |
...
| 4 | Fail-open by construction | 🟡 | Add bounded query timeout + typed `TimeoutError` to local graph backends (none today in `falkordb-shared`/`bolt-shared`). Guarantee never-throw at the integration boundary (wrap the SDK entry, not just per-tool `handleToolErrors`). | `errors.ts`, `tools/error-handling.ts:15-45`, `cloud.ts:171-181` (model), `falkordb-shared.ts`/`bolt-shared.ts` (no timeout) |
...
- [ ] **H8: FalkorDB param-passing bug breaks all parameterized Cypher on the DEFAULT backend** *(→ Tier 0 #4, discovered 2026-07-19)* — `backends/falkordb-shared.ts:99` calls `this.graph.query(query, params)` but the falkordb-ts driver reads `options.params`, so `params` is always `undefined`, no `CYPHER k=v` prefix is emitted, and every parameterized query (`store`/`search`/`recall`/`update`/etc.) fails on falkordblite AND falkordb with "Missing parameters" / "expected STARTS WITH, SET or START". Latent because no test exercises a live falkordblite cycle. Fix: `this.graph.query(query, { params })`. Blocks the D1-Option-C premise.
...
- [ ] **M13: legacy `CREATE CONSTRAINT` schema init on FalkorDB v4.16.3** *(→ Tier 0 #4, discovered 2026-07-19)* — schema init uses legacy `CREATE CONSTRAINT` Cypher; FalkorDB v4.16.3 requires the `GRAPH.CONSTRAINT` command. The error is currently swallowed, so constraints are silently never created. Fix: use `GRAPH.CONSTRAINT` syntax (or `GRAPH.CONSTRAINT CREATE ...`).
...
- [ ] **SEC-5: sensitive data in error messages** *(→ Tier 0 #4)* — debug-log full error, surface generic message.
