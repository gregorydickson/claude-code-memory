...
- [x] ✅ **M7: intelligence/analytics/proactive throw on SQLite & Cloud** *(→ Tier 1 #10, falkordblite parity fixed in M5)* — entities, patterns, context, visualize, similarity, learning, gaps, briefing, predict, warn, outcome all run on the default falkordblite backend without throwing (now that H8 param bug is fixed). `outcome` `datetime()` Cypher bug fixed in `proactive/outcome-learning.ts` (FalkorDB does not implement `datetime()` — ISO strings passed as params instead). Sqlite-fallback `isCypherCapable()` guards + clear messages handled by the sqlite-fallback-messages feature. Tests in `tests/m5-feature-parity.test.ts` (VAL-LOCAL-020..030, VAL-CROSS-006).
